...
export interface DepFields { name: ToField<String>; package: ToField<"address">; version: ToField<"u64"> }
...
export class Dep implements StructClass { __StructClass = true as const;
 ...
 readonly name: ToField<String>; readonly package: ToField<"address">; readonly version: ToField<"u64">
 ...
 this.name = fields.name;; this.package = fields.package;; this.version = fields.version; }
 ...
 name: String.bcs, package: bcs.bytes(32).transform({ input: (val: string) => fromHEX(val), output: (val: Uint8Array) => toHEX(val), }), version: bcs.u64()

}) };

 static fromFields( fields: Record<string, any> ): Dep { return Dep.reified( ).new( { name: decodeFromFields(String.reified(), fields.name), package: decodeFromFields("address", fields.package), version: decodeFromFields("u64", fields.version) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): Dep { if (!isDep(item.type)) { throw new Error("not a Dep type");

 }

 return Dep.reified( ).new( { name: decodeFromFieldsWithTypes(String.reified(), item.fields.name), package: decodeFromFieldsWithTypes("address", item.fields.package), version: decodeFromFieldsWithTypes("u64", item.fields.version) } ) }

 static fromBcs( data: Uint8Array ): Dep { return Dep.fromFields( Dep.bcs.parse(data) ) }

 toJSONField() { return {

 name: this.name,package: this.package,version: this.version.toString(),

} }

 toJSON() { return { $typeName: this.$typeName, $typeArgs: this.$typeArgs, ...this.toJSONField() } }

 static fromJSONField( field: any ): Dep { return Dep.reified( ).new( { name: decodeFromJSONField(String.reified(), field.name), package: decodeFromJSONField("address", field.package), version: decodeFromJSONField("u64", field.version) } ) }
