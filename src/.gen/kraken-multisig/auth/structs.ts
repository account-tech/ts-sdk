...
export interface AuthFields { witness: ToField<TypeName>; name: ToField<String>; multisigAddr: ToField<"address"> }
...
export class Auth implements StructClass { __StructClass = true as const;
 ...
 readonly witness: ToField<TypeName>; readonly name: ToField<String>; readonly multisigAddr: ToField<"address">
 ...
 this.witness = fields.witness;; this.name = fields.name;; this.multisigAddr = fields.multisigAddr; }
 ...
 witness: TypeName.bcs, name: String.bcs, multisig_addr: bcs.bytes(32).transform({ input: (val: string) => fromHEX(val), output: (val: Uint8Array) => toHEX(val), })

}) };

 static fromFields( fields: Record<string, any> ): Auth { return Auth.reified( ).new( { witness: decodeFromFields(TypeName.reified(), fields.witness), name: decodeFromFields(String.reified(), fields.name), multisigAddr: decodeFromFields("address", fields.multisig_addr) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): Auth { if (!isAuth(item.type)) { throw new Error("not a Auth type");

 }

 return Auth.reified( ).new( { witness: decodeFromFieldsWithTypes(TypeName.reified(), item.fields.witness), name: decodeFromFieldsWithTypes(String.reified(), item.fields.name), multisigAddr: decodeFromFieldsWithTypes("address", item.fields.multisig_addr) } ) }

 static fromBcs( data: Uint8Array ): Auth { return Auth.fromFields( Auth.bcs.parse(data) ) }

 toJSONField() { return {

 witness: this.witness.toJSONField(),name: this.name,multisigAddr: this.multisigAddr,

} }

 toJSON() { return { $typeName: this.$typeName, $typeArgs: this.$typeArgs, ...this.toJSONField() } }

 static fromJSONField( field: any ): Auth { return Auth.reified( ).new( { witness: decodeFromJSONField(TypeName.reified(), field.witness), name: decodeFromJSONField(String.reified(), field.name), multisigAddr: decodeFromJSONField("address", field.multisigAddr) } ) }
