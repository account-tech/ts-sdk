...
export interface MultisigFields { id: ToField<UID>; deps: ToField<Deps>; name: ToField<String>; members: ToField<Members>; thresholds: ToField<Thresholds>; proposals: ToField<Proposals> }
...
export class Multisig implements StructClass { __StructClass = true as const;
 ...
 readonly id: ToField<UID>; readonly deps: ToField<Deps>; readonly name: ToField<String>; readonly members: ToField<Members>; readonly thresholds: ToField<Thresholds>; readonly proposals: ToField<Proposals>
 ...
 this.id = fields.id;; this.deps = fields.deps;; this.name = fields.name;; this.members = fields.members;; this.thresholds = fields.thresholds;; this.proposals = fields.proposals; }
 ...
 id: UID.bcs, deps: Deps.bcs, name: String.bcs, members: Members.bcs, thresholds: Thresholds.bcs, proposals: Proposals.bcs

}) };

 static fromFields( fields: Record<string, any> ): Multisig { return Multisig.reified( ).new( { id: decodeFromFields(UID.reified(), fields.id), deps: decodeFromFields(Deps.reified(), fields.deps), name: decodeFromFields(String.reified(), fields.name), members: decodeFromFields(Members.reified(), fields.members), thresholds: decodeFromFields(Thresholds.reified(), fields.thresholds), proposals: decodeFromFields(Proposals.reified(), fields.proposals) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): Multisig { if (!isMultisig(item.type)) { throw new Error("not a Multisig type");

 }

 return Multisig.reified( ).new( { id: decodeFromFieldsWithTypes(UID.reified(), item.fields.id), deps: decodeFromFieldsWithTypes(Deps.reified(), item.fields.deps), name: decodeFromFieldsWithTypes(String.reified(), item.fields.name), members: decodeFromFieldsWithTypes(Members.reified(), item.fields.members), thresholds: decodeFromFieldsWithTypes(Thresholds.reified(), item.fields.thresholds), proposals: decodeFromFieldsWithTypes(Proposals.reified(), item.fields.proposals) } ) }

 static fromBcs( data: Uint8Array ): Multisig { return Multisig.fromFields( Multisig.bcs.parse(data) ) }

 toJSONField() { return {

 id: this.id,deps: this.deps.toJSONField(),name: this.name,members: this.members.toJSONField(),thresholds: this.thresholds.toJSONField(),proposals: this.proposals.toJSONField(),

} }

 toJSON() { return { $typeName: this.$typeName, $typeArgs: this.$typeArgs, ...this.toJSONField() } }

 static fromJSONField( field: any ): Multisig { return Multisig.reified( ).new( { id: decodeFromJSONField(UID.reified(), field.id), deps: decodeFromJSONField(Deps.reified(), field.deps), name: decodeFromJSONField(String.reified(), field.name), members: decodeFromJSONField(Members.reified(), field.members), thresholds: decodeFromJSONField(Thresholds.reified(), field.thresholds), proposals: decodeFromJSONField(Proposals.reified(), field.proposals) } ) }
