...
export interface DestroyArgs { executable: TransactionObjectInput; witness: GenericArg }

export function destroy( tx: Transaction, typeArg: string, args: DestroyArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::executable::destroy`, typeArguments: [typeArg], arguments: [ obj(tx, args.executable), generic(tx, `${typeArg}`, args.witness) ], }) }
...
export interface ActionMutArgs { executable: TransactionObjectInput; witness: GenericArg; multisigAddr: string | TransactionArgument }

export function actionMut( tx: Transaction, typeArgs: [string, string], args: ActionMutArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::executable::action_mut`, typeArguments: typeArgs, arguments: [ obj(tx, args.executable), generic(tx, `${typeArgs[0]}`, args.witness), pure(tx, args.multisigAddr, `address`) ], }) }
...
export function removeAction( tx: Transaction, typeArgs: [string, string], args: RemoveActionArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::executable::remove_action`, typeArguments: typeArgs, arguments: [ obj(tx, args.executable), generic(tx, `${typeArgs[0]}`, args.witness) ], }) }
