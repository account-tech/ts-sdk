...
export function proposeConfigDeps( tx: Transaction, typeArgs: [string, string], args: ProposeConfigDepsArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::config::propose_config_deps`, typeArguments: typeArgs, arguments: [ obj(tx, args.auth), obj(tx, args.account), generic(tx, `${typeArgs[1]}`, args.outcome), pure(tx, args.key, `${String.$typeName}`), pure(tx, args.description, `${String.$typeName}`), pure(tx, args.executionTime, `u64`), pure(tx, args.expirationEpoch, `u64`), obj(tx, args.extensions), pure(tx, args.names, `vector<${String.$typeName}>`), pure(tx, args.packages, `vector<address>`), pure(tx, args.versions, `vector<u64>`) ], }) }
