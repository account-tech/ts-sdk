...
export function cancelPayment( tx: Transaction, typeArgs: [string, string, string], args: CancelPaymentArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::vesting::cancel_payment`, typeArguments: typeArgs, arguments: [ obj(tx, args.auth), obj(tx, args.vesting), obj(tx, args.account) ], }) }
