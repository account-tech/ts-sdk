...
export interface ProposeRestrictArgs { auth: TransactionObjectInput; account: TransactionObjectInput; outcome: GenericArg; key: string | TransactionArgument; description: string | TransactionArgument; expirationEpoch: bigint | TransactionArgument; package: string | TransactionArgument; policy: number | TransactionArgument; clock: TransactionObjectInput }

export function proposeRestrict( tx: Transaction, typeArgs: [string, string], args: ProposeRestrictArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::upgrade_policies::propose_restrict`, typeArguments: typeArgs, arguments: [ obj(tx, args.auth), obj(tx, args.account), generic(tx, `${typeArgs[1]}`, args.outcome), pure(tx, args.key, `${String.$typeName}`), pure(tx, args.description, `${String.$typeName}`), pure(tx, args.expirationEpoch, `u64`), pure(tx, args.package, `address`), pure(tx, args.policy, `u8`), obj(tx, args.clock) ], }) }
...
export function proposeUpgrade( tx: Transaction, typeArgs: [string, string], args: ProposeUpgradeArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::upgrade_policies::propose_upgrade`, typeArguments: typeArgs, arguments: [ obj(tx, args.auth), obj(tx, args.account), generic(tx, `${typeArgs[1]}`, args.outcome), pure(tx, args.key, `${String.$typeName}`), pure(tx, args.description, `${String.$typeName}`), pure(tx, args.expirationEpoch, `u64`), pure(tx, args.package, `address`), pure(tx, args.digest, `vector<u8>`), obj(tx, args.clock) ], }) }
