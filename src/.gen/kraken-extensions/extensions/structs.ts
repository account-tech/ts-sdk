...
export interface HistoryFields { package: ToField<"address">; version: ToField<"u64"> }
...
export class History implements StructClass { __StructClass = true as const;
 ...
 readonly package: ToField<"address">; readonly version: ToField<"u64">
 ...
 this.package = fields.package;; this.version = fields.version; }
 ...
 package: bcs.bytes(32).transform({ input: (val: string) => fromHEX(val), output: (val: Uint8Array) => toHEX(val), }), version: bcs.u64()

}) };

 static fromFields( fields: Record<string, any> ): History { return History.reified( ).new( { package: decodeFromFields("address", fields.package), version: decodeFromFields("u64", fields.version) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): History { if (!isHistory(item.type)) { throw new Error("not a History type");

 }

 return History.reified( ).new( { package: decodeFromFieldsWithTypes("address", item.fields.package), version: decodeFromFieldsWithTypes("u64", item.fields.version) } ) }

 static fromBcs( data: Uint8Array ): History { return History.fromFields( History.bcs.parse(data) ) }

 toJSONField() { return {

 package: this.package,version: this.version.toString(),

} }

 toJSON() { return { $typeName: this.$typeName, $typeArgs: this.$typeArgs, ...this.toJSONField() } }

 static fromJSONField( field: any ): History { return History.reified( ).new( { package: decodeFromJSONField("address", field.package), version: decodeFromJSONField("u64", field.version) } ) }
