...
export interface IssuerFields { accountAddr: ToField<"address">; intentKey: ToField<String>; intentType: ToField<TypeName> }
...
export class Issuer implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V1}::issuer::Issuer`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;
 ...
 readonly accountAddr: ToField<"address">; readonly intentKey: ToField<String>; readonly intentType: ToField<TypeName>
 ...
 this.accountAddr = fields.accountAddr;; this.intentKey = fields.intentKey;; this.intentType = fields.intentType; }
 ...
 account_addr: bcs.bytes(32).transform({ input: (val: string) => fromHEX(val), output: (val: Uint8Array) => toHEX(val), }), intent_key: String.bcs, intent_type: TypeName.bcs

}) };

 static fromFields( fields: Record<string, any> ): Issuer { return Issuer.reified( ).new( { accountAddr: decodeFromFields("address", fields.account_addr), intentKey: decodeFromFields(String.reified(), fields.intent_key), intentType: decodeFromFields(TypeName.reified(), fields.intent_type) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): Issuer { if (!isIssuer(item.type)) { throw new Error("not a Issuer type");

 }

 return Issuer.reified( ).new( { accountAddr: decodeFromFieldsWithTypes("address", item.fields.account_addr), intentKey: decodeFromFieldsWithTypes(String.reified(), item.fields.intent_key), intentType: decodeFromFieldsWithTypes(TypeName.reified(), item.fields.intent_type) } ) }

 static fromBcs( data: Uint8Array ): Issuer { return Issuer.fromFields( Issuer.bcs.parse(data) ) }

 toJSONField() { return {

 accountAddr: this.accountAddr,intentKey: this.intentKey,intentType: this.intentType.toJSONField(),

} }

 toJSON() { return { $typeName: this.$typeName, $typeArgs: this.$typeArgs, ...this.toJSONField() } }

 static fromJSONField( field: any ): Issuer { return Issuer.reified( ).new( { accountAddr: decodeFromJSONField("address", field.accountAddr), intentKey: decodeFromJSONField(String.reified(), field.intentKey), intentType: decodeFromJSONField(TypeName.reified(), field.intentType) } ) }
