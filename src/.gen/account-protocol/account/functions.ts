...
export interface CreateProposalArgs { account: TransactionObjectInput; auth: TransactionObjectInput; outcome: GenericArg; version: TransactionObjectInput; witness: GenericArg; wName: string | TransactionArgument; key: string | TransactionArgument; description: string | TransactionArgument; executionTime: bigint | TransactionArgument; expirationEpoch: bigint | TransactionArgument }

export function createProposal( tx: Transaction, typeArgs: [string, string, string], args: CreateProposalArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::account::create_proposal`, typeArguments: typeArgs, arguments: [ obj(tx, args.account), obj(tx, args.auth), generic(tx, `${typeArgs[1]}`, args.outcome), obj(tx, args.version), generic(tx, `${typeArgs[2]}`, args.witness), pure(tx, args.wName, `${String.$typeName}`), pure(tx, args.key, `${String.$typeName}`), pure(tx, args.description, `${String.$typeName}`), pure(tx, args.executionTime, `u64`), pure(tx, args.expirationEpoch, `u64`) ], }) }
