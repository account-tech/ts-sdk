...
export interface AccountFields<Config extends TypeArgument, Outcome extends TypeArgument> { id: ToField<UID>; metadata: ToField<Metadata>; deps: ToField<Deps>; proposals: ToField<Proposals<Outcome>>; config: ToField<Config> }
...
export class Account<Config extends TypeArgument, Outcome extends TypeArgument> implements StructClass { __StructClass = true as const;
 ...
 readonly id: ToField<UID>; readonly metadata: ToField<Metadata>; readonly deps: ToField<Deps>; readonly proposals: ToField<Proposals<Outcome>>; readonly config: ToField<Config>
 ...
 this.id = fields.id;; this.metadata = fields.metadata;; this.deps = fields.deps;; this.proposals = fields.proposals;; this.config = fields.config; }
 ...
 return Account.reified( typeArgs[0], typeArgs[1], ).new( { id: decodeFromFieldsWithTypes(UID.reified(), item.fields.id), metadata: decodeFromFieldsWithTypes(Metadata.reified(), item.fields.metadata), deps: decodeFromFieldsWithTypes(Deps.reified(), item.fields.deps), proposals: decodeFromFieldsWithTypes(Proposals.reified(typeArgs[1]), item.fields.proposals), config: decodeFromFieldsWithTypes(typeArgs[0], item.fields.config) } ) }
 ...
 id: this.id,metadata: this.metadata.toJSONField(),deps: this.deps.toJSONField(),proposals: this.proposals.toJSONField(),config: fieldToJSON<Config>(this.$typeArgs[0], this.config),

} }

 toJSON() { return { $typeName: this.$typeName, $typeArgs: this.$typeArgs, ...this.toJSONField() } }

 static fromJSONField<Config extends Reified<TypeArgument, any>, Outcome extends Reified<TypeArgument, any>>( typeArgs: [Config, Outcome], field: any ): Account<ToTypeArgument<Config>, ToTypeArgument<Outcome>> { return Account.reified( typeArgs[0], typeArgs[1], ).new( { id: decodeFromJSONField(UID.reified(), field.id), metadata: decodeFromJSONField(Metadata.reified(), field.metadata), deps: decodeFromJSONField(Deps.reified(), field.deps), proposals: decodeFromJSONField(Proposals.reified(typeArgs[1]), field.proposals), config: decodeFromJSONField(typeArgs[0], field.config) } ) }
